...
В R1/R2 нет ручной code generation и нет конфигурации backend. После успешного `pub get` проект готов к запуску. iOS впервые запросит доступ к камере только при открытии сканера QR.
