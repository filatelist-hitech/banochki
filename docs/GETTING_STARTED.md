...
В R1/R2 нет ручной code generation и нет конфигурации backend. После успешного `pub get` проект готов к запуску. iOS впервые запросит доступ к камере при открытии сканера QR или съёмке фото партии, а к фотобиблиотеке — при выборе снимка.
...
7. В карточке партии добавьте два фото: свайп по верхнему блоку должен переключать снимки, а последний появится в превью каталога.
...
Сквозной R1/R2 flow на реальном target:
...
> Удаление приложения или очистка его storage удаляет локальную базу. В R1/R2 нет облачной копии и восстановления с другого устройства.
